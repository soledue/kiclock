...
#if os(iOS)
import UIKit
open class KiClockFaceModern: KiClockFaceProtocol {
    ...
    public func draw(bottom view: UIView) {
        ...
        let hoursClockIndexLayer = { () -> CAShapeLayer in
            ...
            let smallRadius = radius - (radius/5 + clockBorderWidth)
            
            let path = UIBezierPath()
            let offesetRadius: CGFloat = radius - 5
            for i in 0..<12 where (i % 3) != 0 {
                let angle = angle270 + angle30 * Float(i)
